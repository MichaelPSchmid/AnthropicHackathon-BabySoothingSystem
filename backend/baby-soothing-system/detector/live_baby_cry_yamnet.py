...
class BabySoothingAssistant(Agent):
    """Baby-beruhigender Agent mit TCP Communication"""
    
    def __init__(self):
        super().__init__(
            instructions="""Du bist ein sanfter Baby-Beruhigungs-Assistent. 
    ...
    async def _start_soothing(self):
        ...
        try:
            await self.agent_session.generate_reply(
                instructions="Das Baby weint gerade. Beginne sofort mit sanften, beruhigenden Worten. Sprich kontinuierlich und liebevoll, bis das Baby ruhig wird."
            )
        except Exception as e:
            print(f"❌ Fehler beim Generieren der Beruhigung: {e}")
...
async def entrypoint(ctx: agents.JobContext):
    ...
    # Agent Session (nur LLM + TTS, kein STT/VAD!)
    session = AgentSession(
        stt=None,  # ✅ Kein STT - keine Mikrofonaufnahme!
        llm=anthropic.LLM(
    ...
    print("🍼 Baby Soothing Agent gestartet!")
    ...
    await session.generate_reply(
        instructions="Stelle dich kurz als Baby-Beruhigungs-Assistent vor. Erkläre, dass du bereit bist und auf die Verbindung zum Baby-Schrei-Detektor wartest."
    )
